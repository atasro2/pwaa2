...
u32 Command1C(struct ScriptContext * scriptCtx)
{
    ...
    switch(*scriptCtx->scriptPtr)
    {
        case 0: // enable textbox
            ...
            CopyTextboxTilesToBG1MapBuffer();
            SetTextboxNametag(gMain.currentNametagId, gMain.currentNametagSide);
            break;
...
struct SpotSelectData
{
    ...
    /* +0x26 */ u16 unused26; // Unused, called a dummy in unity
    /* +0x28 */ u8 left;
    /* +0x29 */ u8 top;
    /* +0x2A */ u8 right;
    /* +0x2B */ u8 bottom;
...
bool32 Command4D(struct ScriptContext *scriptCtx)
{
    ...
    scriptCtx->scriptPtr++;
    if(*scriptCtx->scriptPtr == 0)
        gMain.disableDetentionCenterMask = 1;
    else
        gMain.disableDetentionCenterMask = 0;
    scriptCtx->scriptPtr++;
    return 1;
}
...
bool32 Command4E(struct ScriptContext *scriptCtx)
{
    ...
    if(scriptCtx->waitTimer != 0) {
        if(scriptCtx->waitingPersonId != gAnimation[1].animationInfo.personId) {
            SetAnimationFrameOffset(&gAnimation[1], gMain.idleAnimationOffset);
            scriptCtx->waitingPersonId = gAnimation[1].animationInfo.personId;
        }
        scriptCtx->waitTimer--;
        if(scriptCtx->waitTimer == 0) {
            scriptCtx->scriptPtr += 2;
            scriptCtx->waitingPersonId = 0;
            return 0;
        }
...
bool32 Command65(struct ScriptContext *scriptCtx)
{
    ...
    scriptCtx->scriptPtr++;
    var1 = *scriptCtx->scriptPtr;
    gMain.courtBenchSpritesTypeFlag = (var0 << 4) | (var1 & 0xF);
    switch(var0) {
        case 0:
            if(var1 == 1) {
...
{
    scriptCtx->scriptPtr++;
    gMain.testimonyLastSection = *scriptCtx->scriptPtr;
    scriptCtx->scriptPtr++;
    return 0;
}
